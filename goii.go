...
func New(logWriter io.Writer, logPrefix string) *Goii {
	if logWriter == nil {
		logWriter = os.Stdout
	}
	if logPrefix == "" {
	...
	m := &Goii{
		...
		action:   func() {},
		Router:   NewRouter(),
		logger:   log.New(logWriter, "["+logPrefix+"] ", 0),
	}
	m.Router.m = m
	m.Map(m.logger)
...
func (m *Goii) Handlers(handlers ...Handler) {
...
func (m *Goii) Action(handler Handler) {
...
func (m *Goii) Before(handler BeforeHandler) {
...
func (m *Goii) Use(handler Handler) {
	handler = validateAndWrapHandler(handler)
	m.handlers = append(m.handlers, handler)
}

func (m *Goii) createContext(rw http.ResponseWriter, req *http.Request) *Context {
	c := &Context{
		Injector: inject.New(),
		handlers: m.handlers,
		action:   m.action,
...
func (m *Goii) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	if m.hasURLPrefix {
...
func (m *Goii) Run(args ...interface{}) {
...
func (m *Goii) SetURLPrefix(prefix string) {
	m.urlPrefix = prefix
