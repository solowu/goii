...
import (
	"github.com/solowu/goii/mod/form"
	"html/template"
	"io"
	"io/ioutil"
	"mime/multipart"
...
var defaultCookieSecret string = "goiicookiekey"

// SetDefaultCookieSecret sets global default secure cookie secret.
func (m *Goii) SetDefaultCookieSecret(secret string) {
...
func (ctx *Context) SetSuperSecureCookie(secret, name, value string, others ...interface{}) {
	ctx.SetCookie(name, Rc4En(value, secret), others...)
}
...
func (c *Context) Json(code int, msg string, data ...interface{}) {
	if len(data) >= 1 {
		c.JSON(200, &ajaxData{Code: code, Msg: msg, Data: data[0]})
	} else {
		c.JSON(200, &ajaxData{Code: code, Msg: msg})
	}
}

// Handle handles and logs error by given status.
func (c *Context) Error(status int, title string, err error) {

	c.Data["Title"] = title
	c.Data["ErrorMsg"] = err
	c.Data["status"] = status
	if c.IsAJAX() {
		c.Json(status, title, err)
		return
	}

	c.HTML(status, "error/error")
}
...
func (c *Context) FormData(data interface{}) error {
	c.parseForm()
	if err := form.NewDecoder().Decode(data, c.Req.Form); err != nil {
		return err
	}
